...
<style type="text/tailwindcss">
    .select2-container .select2-selection--single {
            @apply h-[46px] flex items-center px-2 rounded-xl border border-gray-200 bg-gray-50 outline-none transition-all;
        }
        .select2-container--default .select2-selection--single .select2-selection__arrow {
            @apply h-[44px] right-3;
        }
        .select2-container--default .select2-selection--single:focus,
        .select2-container--default.select2-container--open .select2-selection--single {
            @apply border-blue-500 bg-white ring-2 ring-blue-100;
        }
        .select2-dropdown {
            @apply border-blue-200 shadow-xl rounded-xl overflow-hidden mt-1;
        }
        .select2-search__field {
            @apply rounded-lg border-gray-300 focus:ring-2 focus:ring-blue-200 focus:border-blue-400 outline-none px-3 py-2;
        }
    ...
    <main class="flex-1 overflow-y-auto p-4 sm:p-6 lg:p-8 relative bg-gray-50">
        ...
        <div class="bg-white rounded-2xl shadow-sm border border-amber-200 overflow-hidden mb-8 relative">
            ...
            <div class="p-6 border-b border-amber-100 bg-amber-50/30 flex justify-between items-center">
                <div>
                    ...
                    <p class="text-sm text-amber-600/80">Wali santri telah mengupload bukti transfer, silakan
                        divalidasi.
                    </p>
                </div>
            </div>
                    ...
                    ");
                        ...
                        ?>
                                <tr class="hover:bg-amber-50/20 transition-colors border-b border-gray-50">
                                    ...
                                    <td class="p-4">
                                        <p class="font-bold text-gray-800"><?php echo htmlspecialchars($p['nama_santri']); ?>
                                            <span
                ...
                <form action="kelola_pembayaran.php" method="POST" class="p-6">
                    <div class="space-y-4">
                        ...
                        <div>
                            ...
                            <select name="target_santri" id="select2-santri" required class="w-full">
                                <option value="semua">-- BUAT UNTUK SEMUA SANTRI AKTIF --</option>
                                <optgroup label="Atau Ketik NIS / Nama Santri:">
                                    <?php
                                    // Query-nya diubah untuk mengambil NIS juga
                                    $q_s = mysqli_query($koneksi, "SELECT id_santri, nis, nama_santri, kelas FROM santri WHERE status_aktif='Aktif'");
                                    while ($s = mysqli_fetch_assoc($q_s)) {
                                        // Tampilkan format [NIS] Nama Santri - Kelas
                                        echo "<option value='" . $s['id_santri'] . "'>[" . $s['nis'] . "] " . $s['nama_santri'] . " - Kelas " . $s['kelas'] . "</option>";
                                    }
                                    ?>
                                </optgroup>
                            </select>
        ...
        <div id="modal-cash"
            ...
            <div class="bg-white rounded-2xl w-full max-w-sm shadow-2xl transform scale-95 transition-transform duration-300 text-center p-6"
                ...
                <p class="text-sm text-gray-500 mb-6">Anda akan mencatat pembayaran secara offline/tunai untuk
                    <br><strong id="nama_santri_cash" class="text-gray-800">Nama Santri</strong> (<span
                        id="jenis_tagihan_cash">SPP</span>).
                </p>
...
<script>
    $(document).ready(function() {
        ...
        $('#select2-santri').select2({
            dropdownParent: $('#modal-generate'), // <-- PENTING: Biar bisa diketik di dalam Modal
            width: '100%',
